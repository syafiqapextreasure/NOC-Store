...
import 'package:nocstore/model/WorkingHoursModel.dart';
                                            ...
                                            children: [
                                              Expanded(
                                                ...
                                                onTap: () async {
                                                  ...
                                                  setState(() {
                                                    workingHours[index].timeslot![index1].from = DateFormat('HH:mm').format(DateTime(
                                                        DateTime.now().year, DateTime.now().month, DateTime.now().day, startTime!.hour, startTime.minute));
                                                  });
                                                },
                                                child: Container(
                                              ...
                                              Expanded(
                                                ...
                                                onTap: () async {
                                                  ...
                                                  if (startTime!.format(context).toString() == "12:00 AM") {
                                                    ...
                                                    setState(() {
                                                      workingHours[index].timeslot![index1].to = DateFormat('HH:mm').format(DateTime(
                                                          DateTime.now().year, DateTime.now().month, DateTime.now().day, startTime.hour, startTime.minute));
                                                    });
                                                  }
                                                },
                                                    ...
                                                    child: Padding(
                                                      ...
                                                      child: Text(
                                                        workingHours[index].timeslot![index1].to!.isEmpty
                                                            ? 'End Time'.tr()
                                                            : workingHours[index].timeslot![index1].to.toString(),
                                                        style: TextStyle(
                                                            color: isDarkMode(context)
                                                                ? const Color(0xFFFFFFFF)
                                                                : workingHours[index].timeslot![index1].to!.isEmpty
          ...
          onPressed: () {
            if (MyAppState.currentUser!.vendorID.isEmpty) {
              final snackBar =  SnackBar(
                content: Text('Please add a restaurant first').tr(),
              );
  ...
  saveWorkingHours() async {
    ...
    if (vendorModel != null) {
      ...
      await FireStoreUtils.updateVendor(vendorModel!).then((value) async {
        await showProgress(context, 'Update working hours...', false);
        await hideProgress();
      });
    }
  }
